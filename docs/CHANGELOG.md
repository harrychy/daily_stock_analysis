...
- [新功能] 新增"两阶段股票筛选"工作流：第一阶段技术面 Python 评分（不调 LLM）批量扫描，第二阶段按 Top N 自动跳过当日已分析的股票后调 LLM 综合分析。新 API: `POST /api/v1/screening/technical`、`GET /api/v1/screening/{id}`、`POST /api/v1/screening/{id}/promote`；新前端页 `/two-stage-screening` 从首页空状态进入。详见 `docs/two-stage-screening.md`。
- [修复] SAP AI Core 命名空间下 `anthropic--*-opus*` 模型省略 temperature 参数，避免上游网关返回 `temperature is deprecated for this model`。
